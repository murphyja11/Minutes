...
struct FBAuth {
    ...
    static func resetPassword(email: String, resetCompletion: @escaping (Result<Bool,Error>) -> Void) {
        print("starting reset password function")
        print("\(email)")
    ...
    static func handle(_ signInWithAppleResult: SignInWithAppleResult, completion: @escaping (Result<Bool, Error>) -> ()) {
        ...
        // All subsequent times, the fullName will be nill so you need to capture it now if you want it for your database
        ...
        let data = FBUser.dataDict(uid: uid, name: name, email: email)
